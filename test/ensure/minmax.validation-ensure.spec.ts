...
describe('ENSURE_MINMAX', () => {
  it('should have it\'s default description', () => {
    ...
    let desc = evalStrThunk(ENSURE_MINMAX(5, -1).description);
    expect(desc).to.equal('at least 5 characters');

    // Singular lower, no upper
    desc = evalStrThunk(ENSURE_MINMAX(1, -1).description);
    ...
    desc = evalStrThunk(ENSURE_MINMAX(2, 3).description);
    ...
    desc = evalStrThunk(ENSURE_MINMAX(-1, 3).description);
