...
import { findArrayDuplicate, STAGE_ISUNIQUE } from '../../src/stage/isunique.validation-stage';
import { runStageTesting } from '../test-util';

describe('STAGE_ISUNIQUE', () => {

  it('should accept no-dup arrays case-sensitive', () => {
    const { result } = runStageTesting(STAGE_ISUNIQUE, {
      description: '',
  ...
  it('shouldn\'t accept dup arrays case-sensitive', () => {
    const { control, result } = runStageTesting(STAGE_ISUNIQUE, {
      description: '',
    }, ['i', 'am', 'different', 'different'], { isArray: true, isUnique: true });
